...
# ---- Define input/output type for state -----
...
# ---- Tools (Assume these work, as per your setup) -----
@tool
...
Rewrite it to better match the documents in the context DB.
"""
)

rewrite_context_prompt = PromptTemplate.from_template(
    """
The following information was retrieved from the RAG DB:
"{context}"

Add the dump directory prefix: {dump_dir} to any file paths or file names mentioned.
...
analyze_file_prompt = PromptTemplate.from_template(
    """
You have the following context from the RAG DB:
"""
{context}
"""

Check the necessary files using the tools and extract insights accordingly.
"""
)
...
Give a clear and helpful response combining all this information.
"""
)
...
def retrieve_documents(state: State):
    ...
    return {"retrieved_docs": context, "messages": state["messages"]}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        result = runnable.invoke({"messages": [HumanMessage(content=user_input)], "dump_dir": dump_dir}, config={"configurable": {"session_id": session_id}})
