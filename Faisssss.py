...
FAISS_INDEX_FILE = "cr_faiss_index.pkl"
...
def find_similar_crs(state):
    ...
    query_vector = np.array([get_embedding(jira_text)]).astype("float32")
...
✅ I've updated your code to replace Qdrant with FAISS for embedding and retrieval. You can now run:

python script.py --index to index the CRs into FAISS.

python script.py to run the full comparison workflow.


Let me know if you want to:

Load the FAISS index from disk once at startup and keep it in memory.

Expose this as an API for distributed access.

Add persistent storage and dynamic update support.
