...
class FAClassifier(nn.Module):
    ...
    def forward(self, inputs):
        jira_vec = self.bert(**{k: v.squeeze(0) for k, v in inputs['jira'].items()}).pooler_output
        jira_vec = self.jira_fc(jira_vec)

        if 'fw_log' in inputs:
            fw_vec = self.bert(**{k: v.squeeze(0) for k, v in inputs['fw_log'].items()}).pooler_output
            fw_vec = self.fw_fc(fw_vec)

            callstack_vec = self.bert(**{k: v.squeeze(0) for k, v in inputs['callstack'].items()}).pooler_output
            callstack_vec = self.callstack_fc(callstack_vec)

            combined = torch.cat([jira_vec, fw_vec + callstack_vec], dim=1)

        else:
            dmesg_vec = self.bert(**{k: v.squeeze(0) for k, v in inputs['dmesg'].items()}).pooler_output
            driver_vec = self.bert(**{k: v.squeeze(0) for k, v in inputs['driver'].items()}).pooler_output
...
def train_model(data_path, num_classes, num_epochs=5, batch_size=4):
    ...
    for epoch in range(num_epochs):
        ...
        evaluate_model(model, dataloader, dataset.idx2label, device)
